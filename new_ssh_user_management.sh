...
mkdir -p /etc/rusty_socks_proxy/
touch $SOCKS5_USERS_FILE
...
create_ssh_user() {
    ...
    sudo useradd -m -s /bin/bash "$USERNAME" || { echo "Erro ao criar usuário do sistema."; return 1; }
    echo "$USERNAME:$PASSWORD" | sudo chpasswd || { echo "Erro ao definir senha do usuário do sistema."; return 1; }
    echo "Usuário do sistema $USERNAME criado com sucesso."

    # Adicionar usuário ao arquivo do SOCKS5
    echo "$USERNAME:$PASSWORD" | sudo tee -a $SOCKS5_USERS_FILE > /dev/null || { echo "Erro ao adicionar usuário SOCKS5."; return 1; }
    echo "Usuário SOCKS5 $USERNAME adicionado com sucesso."
...
remove_ssh_user() {
    ...
    sudo userdel -r "$USERNAME" || { echo "Erro ao remover usuário do sistema."; return 1; }
    echo "Usuário do sistema $USERNAME removido com sucesso."

    # Remover usuário do arquivo do SOCKS5
    sudo sed -i "/^$USERNAME:/d" $SOCKS5_USERS_FILE || { echo "Erro ao remover usuário SOCKS5."; return 1; }
    echo "Usuário SOCKS5 $USERNAME removido com sucesso."
...
list_ssh_users() {
    ...
    echo "Usuários do Sistema (SSH):"
    getent passwd | grep -E ":/home/" | cut -d: -f1

    echo "\nUsuários SOCKS5 (do arquivo $SOCKS5_USERS_FILE):"
    if [ -f "$SOCKS5_USERS_FILE" ]; then
        cut -d: -f1 $SOCKS5_USERS_FILE
    else
        echo "Arquivo de usuários SOCKS5 não encontrado."
    fi
}
